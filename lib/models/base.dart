...
class Model {
  String id;
  final _loggerDebug = LoggerDebug(headColor: LogColors.red, constTitle: "Error while parsing");
  // final _firebaseCrashlytics = KiwiContainer().resolve<FirebaseCrashlyticsConfig>();
  Model({this.id = ''});
  bool get hasData => id.isNotEmpty;

  @override
  bool operator ==(other) => other is Model && runtimeType == other.runtimeType && id == other.id;
  ...
  Color colorFromJson(dynamic json, String attribute, {String defaultHexColor = "#000000"}) {
    try {
      ...
      _loggerDebug.red("there are an error in this key : \"\"$attribute\"\" error: $e");
      // _firebaseCrashlytics.jsonToDartRecordError(e, StackTrace.current, attribute, jsonEncode(json ?? {}), "color_from_json");
      return defaultHexColor.color;
    }
  }

  String stringFromJson(dynamic json, String attribute, {String defaultValue = ''}) {
    try {
      if (json == null || json[attribute] == null) {
        return defaultValue;
      } else {
  ...
  DateTime dateFromJson(dynamic json, String attribute, {DateTime? defaultValue, String? parseingFormat}) {
    defaultValue ??= DateTime(0);
    try {
      if (json == null || json[attribute] == null) {
        return defaultValue;
      } else if (parseingFormat != null) {
        return DateFormat(parseingFormat, 'en').parse(json[attribute].toString().replaceAll('Z', ""));
      } else {
  ...
  int intFromJson(dynamic json, String attribute, {int defaultValue = 0}) {
    try {
      if (json == null || json[attribute] == null) {
        ...
        return double.tryParse(json[attribute].toString())?.toInt() ?? json[attribute];
      }
    } catch (e) {
      _loggerDebug.red("there are an error in this key : \"\"$attribute\"\" error: $e");
  ...
  double doubleFromJson(dynamic json, String attribute, {int? fix, double defaultValue = 0.0}) {
    try {
      if (json == null || json[attribute] == null) {
        return defaultValue;
      } else if (fix != null) {
        return double.parse((double.tryParse(json[attribute].toString()) ?? json[attribute]).toStringAsFixed(fix));
      } else {
        return double.parse((double.tryParse(json[attribute].toString()) ?? json[attribute]).toString());
      }
    } catch (e) {
      _loggerDebug.red("there are an error in this key : \"\"$attribute\"\" error: $e");
  ...
  bool boolFromJson(dynamic json, String attribute, {bool defaultValue = false}) {
    try {
      if (json == null || json[attribute] == null) {
        return defaultValue;
      } else if (json[attribute] is bool) {
        return json[attribute];
      } else if ((json[attribute] is String) && ['1', 'true'].contains(json[attribute])) {
        return true;
  ...
  List<T> listFromJson<T>(dynamic json, String attribute, {required T Function(Map<String, dynamic> e) callback}) {
    try {
      List<T> list = <T>[];
  ...
  List<T>? listNullFromJson<T>(dynamic json, String attribute, {required T Function(Map<String, dynamic> e) callback}) {
    try {
